...
We will be using \ref{sigmoid_func} as our activation function. There is some leniancy over which activation function is used. The paper \cite{sibi} found that actually the more important aspect of 
...
\subsection{RPROP+}
Backpropogation is a solid algorithm, and is often used to refer as a blanket term for all the variations of it. In implementing the neural network in this project, we used 
the ``RPROP+'' algorithm \cite{rprop}, short for Resiliant backPROPogation. The $+$ is notation for showing we include backtracking in the algorithm.
...
To outline this algorithm, we give the algorithm as it is given in the paper \cite{rprop}
\begin{algorithm}
